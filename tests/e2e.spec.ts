...
test('Masterwork journey is a dedicated page, not a popup', async ({ page }) => {
  ...
  await expect(page.locator('.workshop-journey-layer')).toHaveCount(0)
  await expect(page.locator('.journey-roadmap-step')).toHaveCount(5)
  await page.locator('.journey-roadmap-step').filter({ hasText: 'Chultan I & II' }).click()
  await expect(page.getByRole('heading', { name: 'Acquire Chultan Masterwork I, then II' })).toBeVisible()
  await expect(page.getByText('Useful numbers without leaving the journey')).toBeVisible()
  await page.reload()
  await expect(page).toHaveURL(/\/journey$/)
  await expect(page.getByRole('heading', { name: 'Masterwork journey', level: 1 })).toBeVisible()
