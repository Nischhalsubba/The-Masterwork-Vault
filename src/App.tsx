...
function Sprite({ index, alt, size = 48 }: { index?: number | null; alt: string; size?: number }) {
  if (index == null) return <span className="sprite fallback" style={{ width: size, height: size }} aria-label={`${alt}, image unavailable`} />
  const cols = catalog.meta.sprite.columns
  const rows = Math.ceil(catalog.meta.sprite.count / cols)
  const col = index % cols
  const row = Math.floor(index / cols)
  return <span className="sprite" role="img" aria-label={alt} style={{ width: size, height: size, backgroundImage: `url(${spriteDataUri})`, backgroundSize: `${cols * 100}% auto`, backgroundPosition: `${cols > 1 ? col / (cols - 1) * 100 : 0}% ${rows > 1 ? row / (rows - 1) * 100 : 0}%` }} />
}
...
function Recipe({ rows }: { rows: { name: string; required: number }[] }) {
  return <div className="recipe-list">{rows.map((r) => { const m = byMaterial.get(norm(r.name)); return <div className="recipe-row" key={r.name}><Sprite index={m?.iconIndex} alt={r.name} size={42} /><div><strong>{r.name}</strong><small>{m?.craftable ? `${m.profession || 'Crafted'} · yields ${m.outputQuantity || 1}` : 'Raw material'}</small></div><b>×{r.required}</b></div> })}</div>
}
...
function Detail({ item, inPlan, togglePlan }: { item: ItemEntry; inPlan: boolean; togglePlan: () => void }) {
  ...
  return <section className="detail panel enter">
    <div className="detail-head"><Sprite index={item.iconIndex} alt={item.name} size={88} /><div className="grow"><div className="pills"><Source value={item.sourceStatus} /><span>{item.profession || item.kind}</span></div><h2>{item.name}</h2><p>{item.classes.includes('All') ? 'Global' : item.classes.join(' · ')} · {item.slot || item.kind}</p></div><button className="primary" onClick={togglePlan}>{inPlan ? 'Remove from plan' : 'Add to plan'}</button></div>
...
function Materials() {
  ...
  return <div className="materials enter"><section className="panel"><label className="search"><Search size={17}/><input aria-label="Search materials" value={q} onChange={e=>setQ(e.target.value)} placeholder="Search materials…"/></label><div className="material-list">{filtered.map(x=><button key={x.name} className={m?.name===x.name?'active':''} onClick={()=>setName(x.name)}><Sprite index={x.iconIndex} alt={x.name} size={40}/><span><strong>{x.name}</strong><small>{x.craftable?`${x.profession} · yield ${x.outputQuantity}`:'Raw material'}</small></span></button>)}</div></section>{m&&<section className="panel"><div className="detail-head"><Sprite index={m.iconIndex} alt={m.name} size={76}/><div><Source value={m.sourceStatus}/><h2>{m.name}</h2><p>{m.craftable?`${m.profession} · output ${m.outputQuantity}`:'Raw / acquired material'}</p></div></div>{recipe?<><h3>Inputs for one craft</h3><Recipe rows={recipe.materials}/></>:<div className="callout"><Gem size={18}/><p>Base material in the current dependency graph.</p></div>}<h3 className="subhead">Used by</h3><div className="tags">{m.usedBy.slice(0,20).map(x=><span key={x}>{x}</span>)}</div></section>}</div>
}
...
export default function App() {
  ...
  const filtered=useMemo(()=>catalog.items.filter(i=>(cls==='All'||i.kind==='Accessory'||i.classes.includes(cls))&&(kind==='All'||i.kind===kind)&&[i.name,i.slot,i.profession,...i.classes,...i.materials.map(m=>m.name)].join(' ').toLowerCase().includes(q.toLowerCase())),[cls,kind,q])
  const item=catalog.items.find(i=>i.id===id)||filtered[0]||catalog.items[0]
