  ...
  return <div className="mw-health-page">
    ...
    <main id="main-content" className="mw-health-main">
      ...
      <section className="mw-health-grid">
        ...
        <article className="mw-health-panel"><div><span className="mw-eyebrow">CATALOG COVERAGE</span><h2>Missing class assignments</h2></div>{report.queues.missingItemClasses.length ? <ul>{report.queues.missingItemClasses.slice(0, 80).map((item) => <li key={item.id}><strong>{item.name}</strong><span>{item.campaign || 'Unknown collection'} · class restriction still needs attributable evidence</span></li>)}</ul> : <p className="mw-health-empty">Every craftable has a class scope.</p>}</article>
        <article className="mw-health-panel"><div><span className="mw-eyebrow">CATALOG COVERAGE</span><h2>Missing item recipes</h2></div>{report.queues.missingItemRecipes.length ? <ul>{report.queues.missingItemRecipes.slice(0, 80).map((item) => <li key={item.id}><strong>{item.name}</strong><span>{item.campaign || 'Unknown collection'} · recipe inputs still need attributable evidence</span></li>)}</ul> : <p className="mw-health-empty">Every catalog craftable has a captured recipe.</p>}</article>
        <article className="mw-health-panel"><div><span className="mw-eyebrow">ASSET INTEGRITY</span><h2>Artwork provenance</h2></div><dl>{Object.entries(artworkCounts).sort(([a],[b]) => a.localeCompare(b)).map(([key,value]) => <div key={key}><dt>{key}</dt><dd>{value}</dd></div>)}</dl><p>Reference-derived visuals preserve layout continuity but are not authentic Neverwinter artwork. Screenshot/game-asset provenance remains separate, and generated references must never be promoted to screenshot-backed.</p></article>
        <article className="mw-health-panel"><div><span className="mw-eyebrow">ACQUISITION COVERAGE</span><h2>Raw-material sourcing</h2></div><strong className="mw-big-number">{report.queues.acquisitionUnknown}</strong><p>of {report.queues.acquisitionTracked} tracked acquisition records have no established route. Published and historical guides are not live-game verification.</p></article>
        ...
        <article className="mw-health-panel"><div><span className="mw-eyebrow">INTENTIONAL EXCLUSIONS</span><h2>Unresolved research fields</h2></div><ul>{report.ignoredKnowledgeGaps.map((gap) => <li key={String(gap)}><strong>{gap}</strong><span>Not treated as confirmed live-game data</span></li>)}</ul></article>
      </section>

      <section className="mw-ledger"><div><span className="mw-eyebrow">VERIFICATION LEDGER</span><h2>Patch-sensitive facts</h2><p>Dates below record literature review, not in-game observation. Open the source and check the current tooltip before relying on patch-sensitive values.</p></div><div className="mw-ledger-table-wrap"><table><thead><tr><th>Fact</th><th>Value</th><th>Status</th><th>Reviewed</th></tr></thead><tbody>{verificationLedger.map((entry) => <tr key={entry.id}><th>{entry.label}{entry.sourceUrl && <a href={entry.sourceUrl} target="_blank" rel="noopener noreferrer"> Read source<span className="sr-only"> (opens in a new tab)</span></a>}{entry.note && <small>{entry.note}</small>}</th><td>{entry.value}</td><td><span className={`mw-status-chip ${entry.status}`}>{entry.status}</span></td><td>{entry.lastVerified}</td></tr>)}</tbody></table></div></section>
    </main>
  </div>
}
