...
import { getMaterialSourceGuide, MATERIAL_SOURCES_NOTICE, normalizeMaterialSourceName, type AcquisitionStatus, type SourceEvidence } from '../data/materialSources'
...
// Each instance has unique labels; Escape closes only the topmost source dialog.
function SourcesDialog({ title, onClose, children }: { title: string; onClose: () => void; children: ReactNode }) {
  ...
  return createPortal(<dialog ref={ref} className="acquisition-dialog" aria-labelledby={titleId} onCancel={(event) => { event.preventDefault(); onClose() }} onKeyDown={(event) => { if (event.key === 'Escape') event.stopPropagation() }} onClick={(event) => { if (event.target !== event.currentTarget) return; const rect = event.currentTarget.getBoundingClientRect(); if (event.clientX < rect.left || event.clientX > rect.right || event.clientY < rect.top || event.clientY > rect.bottom) onClose() }}>
...
export function MaterialSourceBrowser({ names }: { names: string[] }) {
  ...
  const guides = useMemo(() => [...new Map(names.map((name) => [normalizeMaterialSourceName(name), getMaterialSourceGuide(name)])).values()].sort((a, b) => a.name.localeCompare(b.name)), [names])
  const visible = guides.filter((guide) => (status === 'all' || guide.status === status) && [guide.name, guide.scope, ...guide.routes.map((route) => `${route.location} ${route.method}`)].join(' ').toLowerCase().includes(query.trim().toLowerCase()))
