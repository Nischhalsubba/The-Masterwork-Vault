...
for (const [name,slot] of [
  ...
  assert.equal(item.recipeKnown,true)
  assert.equal(item.sourceStatus,'publisher-plus-community-reconciled')
  assert.equal(item.levelRequirement,20)
  assert.ok(item.profession)
