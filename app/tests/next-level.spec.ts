...
test('readiness tracks seven professions and survives reload', async ({ page }) => {
  ...
  await chultan.click()
  await page.reload()
  await expect(page.getByRole('spinbutton', { name: 'Alchemy level' })).toHaveValue('20')
  await expect(page.getByRole('button', { name: 'Chultan I for Alchemy' })).toHaveAttribute('aria-pressed', 'true')
})
